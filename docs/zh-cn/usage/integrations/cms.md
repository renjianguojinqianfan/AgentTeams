...
| 场景 | 配置位置 |
|------|----------|
| **嵌入式 Docker** | 将所有 `AGENTTEAMS_CMS_*` 配在 **`agentteams-manager`**（若使用 QwenPaw Manager 则为 **`agentteams-manager-qwenpaw`**）上，由 `agentteams-manager.env` 注入。**`agentteams-controller`** 一般不需要为 Agent 可观测性单独配置 CMS 环境变量。 |
