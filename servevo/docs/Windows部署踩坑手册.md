...
| **长时间停机后全局房间损坏（8/13 实测补充）** | 停机 21h 后重启，**所有房间（团队房/各 DM/manager 房）发消息全报 `non-create event for room of unknown version`**，controller 重启无效 | 新建全新房间测试可区分（新房间正常=Tuwunel DB 完好，仅旧房间损坏）。解法：**整体重建 servevo 团队**——`agt delete worker servevo-{leader,cs,qc,coach}` + `agt delete team servevo`（删除后残留容器用 `docker rm -f` 清）→ 重新 `agt apply -f` 4 份 worker YAML + team.yaml → 等 `AgentConfigWatcher started` 后走「五.4」hash 变更法拉起 channel → 新团队房可用 |
