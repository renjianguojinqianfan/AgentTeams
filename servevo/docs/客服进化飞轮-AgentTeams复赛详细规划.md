...
> 版本 v3.3 · 2026-08-13 · v3.3 变化：量化对照演示实现状态注记（§5，无设计变更；8/13 闭环真跑达标 + B4 kp 修复 + B5 全量对照就绪）
...
> **实现状态注记（v3.3，2026-08-13，无设计变更）**：
> - `scripts/closedloop_e2e.py` 已实现并真跑达标（qwen3.7-flash，8/13）：3 题采样（2 普通+1 缺口）跑通 应答→质检→陪练修订→审批→v2 物化发布→复测 全链路，v1→v2 解决率 0.67→1.0 / 转人工 0.33→0 / 质检分 91→95 / 回归通过率 0.67→1.0，判定进化达标；**指标全部由真实应答推导**（快照含实际应答与 signals 可对质），v2 知识包真实物化（`tmp/closedloop-kb-v2`）、顶层含缺失证据段；`verify.sh` 增「闭环脚本诚实性检查」防硬编码指标回潮。
> - `scripts/full50_compare.py` 已实现并真跑（B5，8/13，qwen3.7-plus，top_k=3，4 轮稳定）：v1→v2 解决率 0.88→0.92 / 转人工 0.12→0.08 / 质检代理分 84→86 / 回归通过率 0.76→0.84；**v2 回归通过率未达 0.95（诚实报告）**，剩余为真实局限：检索失败（P08/F02/R10）、应答不完整（R02/F08）、覆盖不足（R11），逐题可见 `tmp/full50-verify/2026-08-13-full50.json` cases_v1/cases_v2。
> - 指标口径修复（8/13）：runner `resolved` 判定从「答案含转人工子串即未应答」改为「短句+兜底措辞开头才算未应答」（`_is_escalation_only`）——排查类题目的正确答案常以「仍无法解决→转人工」收尾，且测试集 kp 本就把转人工列为合法要点，原判定自相矛盾；该修复使 F01/F03/F04/F07/R13/R14 等约 9 题从误判翻正（v2 通过率 0.70→0.84）。
> - 测试集 kp 数据修复（B4，8/13）：**26 处** kp 措辞改为知识来源可提取的语义要点（含真跑暴露的 F01 `查水位`→`水位`、R09、R15）+ v1 知识包补齐 2 处覆盖缺口（超保维修付费、退换货疑问引导）；离线确定性验收 50/50 可提取 + 5/5 缺口语义保真。
> - 人机回环效率：闭环脚本以 `human_approvals/approval_response_ms` 记录（本次为脚本内联审批，真人时长未采集，已在缺失证据段声明）。
