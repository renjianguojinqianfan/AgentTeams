...
- **Declarative MCP on CRDs** — `spec.mcpServers` on Worker, Manager, and Team CRs is now a list of `{name, url, transport}` entries. The controller no longer performs MCP gateway authorization for these servers; `mcporter-servers.json` is written from the CRD (see `docs/usage/resource-management.md`). **Breaking change** for existing manifests that used the old shape.
...
- **CRD 上的声明式 MCP** — Worker、Manager、Team 的 `spec.mcpServers` 现为 `{name, url, transport}` 列表；控制器不再为这些条目做 MCP 网关侧授权；`mcporter-servers.json` 由 CRD 写入（见 `docs/usage/resource-management.md`）。旧字段形态**不兼容**，需更新清单。
